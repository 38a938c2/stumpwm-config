...
(push
  (lambda (w)
    (when (and (window-title w)
               ...
               (title-re-p w "\[[0-9]+\]")
               (not (eq w (current-window))))
      (cl-ppcre:regex-replace " [-–—] (Mozilla Firefox|Librewolf|Firefox Developer Edition)" (window-title w) "")))
  *window-reporting*)
(push
  (lambda (w)
    (when (and (window-title w)
               (title-re-p w "^([·∘•*]|[(][0-9]*[)]) Discord [|] .* [-–—] (Mozilla Firefox|Libre[Ww]olf|Firefox Developer Edition)")
               (not (eq w (current-window))))
      "Discord [*]"))
  *window-reporting*)
(push

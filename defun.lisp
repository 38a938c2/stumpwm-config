...
(defcommand
  ...
  (let*
    ((w (current-window))
     ...
     (s
       (format
         nil "/tmp/ff.~a/sockets/~a/marionette-socket"
         (uiop:getenv "USER")
         (cl-ppcre:regex-replace "[.].*" ps "")))
     )
    (when (probe-file s) s)))
...
(defcommand 
  ...
  (act-on-matching-windows
    ...
    (setf (window-tags w)
          (union
            (window-tags w)
            (windowtags::string-split-by-spaces tags)
            :test 'equalp))))

(defcommand 
  untag-frame-windows (tags) ((:rest "Tags to remove: "))
  (act-on-matching-windows
    (w :frame) t
    (setf (window-tags w)
          (set-difference
...
(defcommand 
  tag-frame-windows-only (tags) ((:rest "Tags to set: "))
  (act-on-matching-windows
    (w :frame) t
    (setf (window-tags w)
          (windowtags::string-split-by-spaces tags))))
...
(defcommand 
  tag-frame-windows-no-auto (tags) ((:rest "Tags to set: "))
  (act-on-matching-windows
    (w :frame) t
    (setf (window-tags w)
          (union
